...
class vLLMHttpServerBase:
    ...
    async def generate(
        ...
        # Handle case where prompt is too long (can happen in multi-turn rollout)
        if max_tokens <= 0:
            logger.warning(
                f"Prompt length ({len(prompt_ids)}) exceeds or equals max_model_len ({self.config.max_model_len}). "
                f"Returning empty output for request {request_id}."
            )
            return TokenOutput(
                token_ids=[],
                log_probs=None,
